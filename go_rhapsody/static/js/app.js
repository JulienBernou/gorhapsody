...
function displayMoveAnalysis(report) {
    ...
    let patterns = [];
    if (report.type === 'Capture') patterns.push(`Captured ${report.captured_count} stone(s)!`);
    if (report.self_atari) patterns.push("Self-Atari!");
    if (report.atari_threats && report.atari_threats.length > 0) patterns.push("Atari Threat!");
    if (report.is_contact_play) patterns.push("Contact Play.");
    if (report.is_hane) patterns.push("Hane.");
    if (report.is_cut) patterns.push("Cut."); // Assuming 'is_cut' exists in your report
    if (report.is_connection) patterns.push("Connection."); // Assuming 'is_connection' exists in your report
    if (report.is_empty_triangle) patterns.push("Empty Triangle."); // Assuming 'is_empty_triangle' exists
    // Add more conditions for other patterns (e.g., is_bad_shape, is_good_shape, is_tesuji, is_joseki)
    // Ensure these properties are being generated by your backend GameAnalyzer for the report object.

    if (patterns.length > 0) {
        analysisText += `patterns: ${patterns.join(", ")}`;
    } else {
        // Provide a default type display if no specific patterns, ensuring it's not empty for non-pass moves
